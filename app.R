...
body <- dashboardBody(
  ...
  tabItems(
    ...
    tabItem(
      tabName = 'pf_strategy',
      tabBox(
        id='bm_box', width=12, status='primary', type='tabs',
        ### a. 자산배분 ====
        tabPanel(
          title="자산배분",
          fluidRow(
            ...
            column(
              ...
              box(
                ...
                solidHeader = TRUE, collapsible = F,
                fluidRow(
                  column(3, uiOutput('allo_year'))
                ),
                fluidRow(
                  uiOutput('allo_table_ui')
                )
              )
            )
          )
        ...
        tabPanel(
          ...
          fluidRow(
            column(width = 12,
                   airDatepickerInput(
                     inputId = "base_month", label = "기준 연월 선택",
                     value = Sys.Date(), view = "months", minView = "months",
                     dateFormat = "yyyy-MM", width = "300px", addon = "right"
                   )
            )
          ),
  ...
  observeEvent(auth_rv$authenticated, {
   ...
   ## b. 자산군별 손익현황====
    ...
    observeEvent(input$allo_add, {
      ...
      next_id <- ifelse(nrow(df_current) == 0, 1, max(as.numeric(df_current$행번호), na.rm = TRUE) + 1)
    ...
    observeEvent(input$allo_modi, {
      ...
      dbxUpdate(conn = ma$con, table = "allo_table", records = mod_row, where_cols = c("행번호"))
      sk_b(!sk_b())
    ...
    observeEvent(input$allo_del, {
      ...
      dbxDelete(conn = ma$con, table = "allo_table", where = data.frame(행번호 = as.numeric(input$allo_new)))
      updateSelectInput(session, "allo_new", selected = "신규")
      sk_b(!sk_b())
      show_delay("자산배분 내역이 삭제되었습니다.", "success")
    })
    
    ### * UI Render (입력창, 연도조회, 테이블)====
    output$allo_input <- renderUI({
      column(
        width=12,
        fluidRow(
          column(width=6,
                 airDatepickerInput('allo_date', "배분일자", addon="none", value=Sys.Date(), width='100%'),
                 autonumericInput('allo_stock_dom', "국내주식", value=0, width='100%', decimalPlaces=3),
                 autonumericInput('allo_bond_mat', "만기보유채권", value=0, width='100%', decimalPlaces=3),
                 autonumericInput('allo_alter_real', "실물자산", value=0, width='100%', decimalPlaces=3),
                 selectInput('allo_new', "신규/수정", choices="신규", width='100%')
          ),
          column(width=6,
                 selectInput('allo_mode', "구분", choices=c("SAA","TAA1","TAA2"), width='100%'),
                 autonumericInput('allo_stock_ovs', "해외주식", value=0, width='100%', decimalPlaces=3),
                 autonumericInput('allo_bond_mkt', "시장형채권", value=0, width='100%', decimalPlaces=3),
                 autonumericInput('allo_alter_inc', "인컴자산", value=0, width='100%', decimalPlaces=3),
                 br(),
                 fluidRow(
                   column(width=4, actionButton("allo_add", "추가", status="info", width='100%')),
                   column(width=4, actionButton("allo_modi", "수정", status="success", width='100%')),
                   column(width=4, actionButton("allo_del", "삭제", status="primary", width='100%'))
                 )
          )
        )
      )
    ...
    output$allo_year <- renderUI({
      ...
      y <- df %>% arrange(desc(배분일자)) %>% mutate(배분일자=year(as.Date(배분일자))) %>% pull(배분일자) %>% unique()
      selectInput('allo_year_select', "조회 연도", choices=y, selected=y[1], width='100%')
    })
    ...
    output$allo_table_ui <- renderUI({
      ...
      df %>% filter(year(as.Date(배분일자)) == as.numeric(input$allo_year_select)) %>% 
        select(행번호, 배분일자, 구분, 국내주식, 해외주식, 만기보유채권, 시장형채권, 실물자산, 인컴자산) %>% 
        mutate(현금성 = 1 - (국내주식 + 해외주식 + 만기보유채권 + 시장형채권 + 실물자산 + 인컴자산)) %>%
        arrange(배분일자, 행번호) %>% 
        flextable() %>% theme_vanilla() %>% colformat_double(j=4:10, digits=3) %>% 
        set_table_properties(layout='autofit') %>% htmltools_value(ft.align='center')
    })
    ...
    raw_bm_data <- reactive({
      ...
      # "평가금액 재계산" 버튼 또는 sk_v 상태 변경시 자동으로 크롤링 및 계산 수행
    ...
    output$dynamic_boxes <- renderUI({
      ...
      fluidRow(
        box(title=paste0(month(t_date), "월 MTD (BM vs MyPF)"), width=6, status="primary", solidHeader=T, plotOutput("plot_mtd_bm")),
        box(title=paste0(month(t_date), "월 MTD (BMPF vs MyPF)"), width=6, status="primary", solidHeader=T, plotOutput("plot_mtd_pf")),
        box(title=paste0(quarter(t_date), "분기 QTD (BM vs MyPF)"), width=6, status="info", solidHeader=T, plotOutput("plot_qtd_bm")),
        box(title=paste0(quarter(t_date), "분기 QTD (BMPF vs MyPF)"), width=6, status="info", solidHeader=T, plotOutput("plot_qtd_pf")),
        box(title=paste0(year(t_date), "년 YTD (BM vs MyPF)"), width=6, status="success", solidHeader=T, plotOutput("plot_ytd_bm")),
        box(title=paste0(year(t_date), "년 YTD (BMPF vs MyPF)"), width=6, status="success", solidHeader=T, plotOutput("plot_ytd_pf"))
      )
    })
    ...
    output$plot_mtd_bm <- renderPlot({ ma_v()$plot_pf_return(raw_bm_data(), cols_bm, floor_date(ma_v()$get_target_date(input$base_month), "month") - days(1)) })
    output$plot_mtd_pf <- renderPlot({ ma_v()$plot_pf_return(raw_bm_data(), cols_pf, floor_date(ma_v()$get_target_date(input$base_month), "month") - days(1)) })
    output$plot_qtd_bm <- renderPlot({ ma_v()$plot_pf_return(raw_bm_data(), cols_bm, floor_date(ma_v()$get_target_date(input$base_month), "quarter") - days(1)) })
    output$plot_qtd_pf <- renderPlot({ ma_v()$plot_pf_return(raw_bm_data(), cols_pf, floor_date(ma_v()$get_target_date(input$base_month), "quarter") - days(1)) })
    output$plot_ytd_bm <- renderPlot({ ma_v()$plot_pf_return(raw_bm_data(), cols_bm, floor_date(ma_v()$get_target_date(input$base_month), "year") - days(1)) })
    output$plot_ytd_pf <- renderPlot({ ma_v()$plot_pf_return(raw_bm_data(), cols_pf, floor_date(ma_v()$get_target_date(input$base_month), "year") - days(1)) })
