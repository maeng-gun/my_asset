...
mod_total_trade_ui <- function(id) {
  ...
  tabPanel(
    title = "종합거래내역",
    box(
      id = ns('total_trade_box'), width = 12, status = 'info',
      solidHeader = TRUE, title = "입력사항", collapsible = FALSE,
      fluidRow(
        column(width = 2, class = "col-12 col-md-4 col-lg-2",
               airDatepickerInput(ns('total_trade_date'), label = "거래일자",
                                  range = TRUE, addon = "none",
                                  value = c(Sys.Date(), Sys.Date()))),
        column(width = 2, class = "col-12 col-md-4 col-lg-2",
               selectInput(ns('total_ass1'), label = "자산군",
                           choices = c("전체", "주식", "대체자산", "채권", "현금성"))),
        column(width = 2, class = "col-12 col-md-4 col-lg-2",
               selectInput(ns('total_ass2'), label = "세부자산군",
                           choices = c("전체", "선진국", "국내", "신흥국", "실물자산",
                                       ...
                                       "만기회사채", "금융상품", "현금"))),
        column(width = 2, class = "col-12 col-md-4 col-lg-2",
               selectInput(ns('total_ass3'), label = "세부자산군2",
                           choices = c("전체", "인덱스", "종목", "테마", "귀금속", "원자재",
                                       "에너지", "국내", "해외", "안전자산", "크레딧",
                                       "부동산", "인프라", "선진국", "신흥국", "단기ETF",
                                       "원화상품", "외화상품", "외환", "원화"))),
        column(width = 2, class = "col-12 col-md-4 col-lg-2",
               selectInput(ns('total_curr'), label = "통화",
